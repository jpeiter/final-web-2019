...
public class BrandController extends CrudController<Brand, Long> {
    ...
    @PostMapping("ajax")
    public ResponseEntity<?> save(@Valid Brand entity, BindingResult result) {
        if (result.hasErrors()) {
    ...
    @GetMapping("ajax/{id}")
    @ResponseBody
    public Brand edit(@PathVariable Long id) {
    ...
    public ModelAndView list(@RequestParam("page") Optional<Integer> page,
        ...
        Page<Brand> list = this.getService().findAll(PageRequest.of(currentPage - 1, pageSize));
